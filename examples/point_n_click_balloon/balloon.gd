...
@onready var responses_menu := $Background/Responses
@onready var response_template := $Background/ResponseTemplate
...
func isVisible() -> bool:
	return dialogue_label.visible || $Background.visible
...
func _on_response_focus_entered(item):
	item.set("theme_override_colors/default_color", COLORS["Jeffery"])
